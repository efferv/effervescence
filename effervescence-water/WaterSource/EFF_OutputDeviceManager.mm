...
// but if this class fails to read/parse it, EFFApp will continue without it.
NSString* const kAudioSystemSettingsPlist =
    @"/Library/Preferences/Audio/com.apple.audio.SystemSettings.plist";

@implementation EFFOutputDeviceManager {
    NSRecursiveLock* _stateLock;

    // Used to change EFFApp's output device.
    EFFAudioDeviceManager* _audioDeviceManager;

    // User settings and data.
//    EFFUserDefaults* _userDefaults;
...
- (instancetype) initWithDevices:(EFFAudioDeviceManager*)audioDeviceManager {
...
//        _userDefaults = userDefaults;
        ...
        DebugMsg("EFFPreferredOutputDevices::initWithDevices: Preferred devices: %s",
...
// Reads the preferred devices list from CoreAudio's Plist file. Uses EFFApp's stored list to fill
...
- (NSArray<NSString*>*) readPreferredDevices {
    ...
    // EFFApp's stored preferred devices to fill in the rest optimistically. This doesn't help us
    ...
    //       warning.
    EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, [&] {
        EFFAudioDevice defaultDevice = CAHALAudioSystemObject().GetDefaultAudioDevice(false, false);
        NSString* __nullable defaultDeviceUID =
            (__bridge_transfer NSString* __nullable)defaultDevice.CopyDeviceUID();

        if (defaultDeviceUID) {
            preferredOutputDeviceInfos = @[ @{ @"uid": EFFNN(defaultDeviceUID) } ];
        }
    });
    
    if (error || !data || !settings) {
        // The Plist file either doesn't exist or we weren't able to parse it.
        LogWarning("EFFPreferredOutputDevices::readPreferredDevices: Couldn't read %s. "
        ...
        // The Plist doesn't include the lists of preferred devices (for input, output and system output).
        LogWarning("EFFPreferredOutputDevices::readPreferredDevices: No preferred devices in %s",
                   settings.debugDescription.UTF8String);
    } else if (!settings[@"preferred devices"][@"output"]) {
        // The Plist doesn't include the list of preferred output devices.
        LogWarning("EFFPreferredOutputDevices::readPreferredDevices: "
                   "No preferred output devices in %s",
                   settings.debugDescription.UTF8String);
    } else {
        // Copy the preferred devices out of the Plist.
        preferredOutputDeviceInfos = EFFNN(settings[@"preferred devices"][@"output"]);
    }
...
//    int storedPreferredDeviceIdx = 0;
    ...
    for (NSDictionary* deviceInfo in deviceInfos) {
        ...
        if (![deviceInfo[@"uid"] isKindOfClass:NSString.class]) {
            LogWarning("EFFPreferredOutputDevices::preferredDeviceUIDsFrom: No UID in %s",
        ...
        if ([uid isEqualToString:@kEFFDeviceUID] ||
            [uid isEqualToString:@kEFFDeviceUID_UISounds] ||
            [uid isEqualToString:@kEFFNullDeviceUID]) {
            // This is one of the Background Music devices, so look for a preferred device saved
            // from a previous run of EFFApp and add it instead.
            //
            // EFFApp has to set EFFDevice, and often also the Null Device for a short time, as the
            ...
            // safe.
            //
            // TODO: This doesn't work if the user has made EFFDevice the systemwide default device
            //       themselves since the last time they closed EFFApp. We might be able to fix that
            //       by having a background process watch the Plist for changes while EFFApp is
...
- (NSArray<NSString*>*) addNextStoredPreferredDevice:(int*)storedPreferredDeviceIdx
    ...
    } while (storedPreferredDevice && [deviceUIDs containsObject:EFFNN(storedPreferredDevice)]);

    // If we found a stored UID, add it to the list.
    if (storedPreferredDevice) {
        DebugMsg("EFFPreferredOutputDevices::addNextStoredPreferredDevice: "
                 "Adding stored preferred device: %s",
                 storedPreferredDevice.UTF8String);
        deviceUIDs = [deviceUIDs arrayByAddingObject:EFFNN(storedPreferredDevice)];
    }
...
- (void) listenForDevicesAddedOrRemoved {
    ...
    EFFOutputDeviceManager* __weak weakSelf = self;
...
- (void) connectedDeviceListChanged {
    @try {
        ...
        if (preferredDevice == kAudioObjectUnknown) {
            LogWarning("EFFPreferredOutputDevices::connectedDeviceListChanged: "
                       "No preferred device found.");
        } else if (_audioDeviceManager.outputDevice.GetObjectID() == preferredDevice) {
            DebugMsg("EFFPreferredOutputDevices::connectedDeviceListChanged: "
            ...
            DebugMsg("EFFPreferredOutputDevices::connectedDeviceListChanged: "
                     ...
                     preferredDevice);
            NSError* __nullable error = [_audioDeviceManager setOutputDeviceWithID:preferredDevice
                                                                   revertOnFailure:YES];
            if (error) {
                // There's not much we can do if this happens.
                LogError("EFFPreferredOutputDevices::connectedDeviceListChanged: "
...
- (AudioObjectID) findPreferredDevice {
    ...
    EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, [&] {
        EFFAudioDevice defaultDevice = audioSystem.GetDefaultAudioDevice(false, false);

        if (!defaultDevice.IsEFFDeviceInstance()) {
            // EFFDevice isn't the systemwide default device, so we know the default device is the
            // most-preferred device that's currently connected.
    ...
    if (preferredDevice == kAudioObjectUnknown) {
        // EFFDevice is the systemwide default device, so this method is probably being called after
        // launch, since we set EFFDevice as the default device then. It could also be that the user
        // set it manually or that EFFApp failed to change it back the last time it closed. Either
        // way, we'll try to find a device to use in the Plist or stored list instead.
        DebugMsg("EFFPreferredOutputDevices::findPreferredDevice: Checking Plist and stored list.");
    ...
    EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, [&] {
        if (preferredDevice == kAudioObjectUnknown && [self isCurrentOutputDeviceConnected]) {
            // The output device (for EFFApp) has been set and is still connected. We haven't found
            // a better device to use, so prefer leaving the output device as it is.
            DebugMsg("EFFPreferredOutputDevices::findPreferredDevice: "
                     "Choosing the current output device as the preferred device.");
            preferredDevice = _audioDeviceManager.outputDevice.GetObjectID();
        }
    });
    ...
    if (preferredDevice == kAudioObjectUnknown) {
        ...
        DebugMsg("EFFPreferredOutputDevices::findPreferredDevice: Choosing an arbitrary device.");
        EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, [&] {
...
// CoreAudio's Plist and EFFApp's stored list.
- (AudioObjectID) findPreferredDeviceInDerivedList {
    ...
    for (int position = 0; position < _preferredDeviceUIDs.count; position++) {
        ...
        for (UInt32 i = 0; i < numDevices; i++) {
            ...
            EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, [&] {
                // Skip devices we can't use, e.g. EFFDevice.
                if (EFFAudioDevice(devices[i]).CanBeOutputDeviceInEFFApp()) {
            ...
            if ([connectedDeviceUID isEqualToString:_preferredDeviceUIDs[position]]) {
                ...
                DebugMsg("EFFPreferredOutputDevices::findPreferredDeviceInDerivedList: "
...
- (bool) isCurrentOutputDeviceConnected {
    if (_audioDeviceManager.outputDevice.GetObjectID() == kAudioObjectUnknown) {
        DebugMsg("EFFPreferredOutputDevices::isCurrentOutputDeviceConnected: "
    ...
    for (UInt32 i = 0; i < numDevices; i++) {
        ...
        if (_audioDeviceManager.outputDevice.GetObjectID() == devices[i]) {
            DebugMsg("EFFPreferredOutputDevices::isCurrentOutputDeviceConnected: "
...
// choose the output device for EFFApp to use.
- (AudioObjectID) findPreferredDeviceByLatency {
    ...
    for (UInt32 i = 0; i < numDevices; i++) {
        EFFAudioDevice device(devices[i]);

        if (!device.IsEFFDeviceInstance()) {
            BOOL hasOutputChannels = NO;

            EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, "GetTotalNumberChannels", [&] {
                hasOutputChannels = device.GetTotalNumberChannels(/* inIsInput = */ false) > 0;
            });

            if (hasOutputChannels) {
                EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, "GetLatency", [&] {
        ...
        EFF_Utils::LogAndSwallowExceptions(EFFDbgArgs, [&] {
            ...
            if (outputDeviceUID) {
                ...
                if (_preferredDeviceUIDs.count >= 2) {
                    _preferredDeviceUIDs = @[EFFNN(outputDeviceUID),
                                          _preferredDeviceUIDs[0],
                                          _preferredDeviceUIDs[1]];
                } else if (_preferredDeviceUIDs.count >= 1) {
                    _preferredDeviceUIDs = @[EFFNN(outputDeviceUID), _preferredDeviceUIDs[0]];
                } else {
                    _preferredDeviceUIDs = @[EFFNN(outputDeviceUID)];
                }

                DebugMsg("EFFPreferredOutputDevices::userChangedOutputDeviceTo: "
                         "Preferred devices: %s",
                         _preferredDeviceUIDs.debugDescription.UTF8String);
                ...
                LogWarning("EFFPreferredOutputDevices::userChangedOutputDeviceTo: "
